...
function Progress() {
    ...
    function createMarks(){
        ...
        for(let i = 1; i <= state.progress.max; i++){
            ...
            marks.push(<div className="progress__mark z-20 text-2xl text-white bg-gray-500" key={i} style={{left: i / state.progress.max * 100 + "%"}}><Icon/></div>);
    ...
    return (
        <div className="hidden md:block absolute w-full bottom-0">
            ...
            <div className="w-11/12 relative ml-8">
                <span className="progress__dot bg-gray-700" style={{left: state.progress.percent + "%"}}/>
